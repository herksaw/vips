...
class BlockRule:
    ...
    """
    relatively large font size for title
    """
    @staticmethod
    def titleRule1(block):
            ...
            if box.nodeType == 3:
                if box.parentNode.nodeName != "script" and box.parentNode.nodeName != "noscript" and box.parentNode.nodeName != "style":
                    if not box.nodeValue.isspace() or box.nodeValue == None:
                            #print("Text= ", box.nodeValue)
                            if float(box.visual_cues['font-size'].replace('px','')) > float(BlockRule.max_font_Size.replace('px','')):
                                BlockRule.max_font_Size = box.visual_cues['font-size']
        for box in block.boxs:
            #adding score based on font size
            if box.nodeType == 3:
                if box.parentNode.nodeName != "script" and box.parentNode.nodeName != "noscript" and box.parentNode.nodeName != "style":
                    if not box.nodeValue.isspace() or box.nodeValue == None:                  
                        if float(box.visual_cues['font-size'].replace('px','')) >= 0.8*float(BlockRule.max_font_Size.replace('px','')):
                              block.titleScore += 2

    """
    title tags e.g. h1, h2, b, strong
    """
    @staticmethod
    def titleRule2(block):
        for box in block.boxs:
            if box.nodeType == 3:
                if box.parentNode.nodeName == "h1":
                    block.titleScore += 3
                elif box.parentNode.nodeName == "h2":
                    block.titleScore += 2
                elif box.parentNode.nodeName == "b" or box.parentNode.nodeName == "strong":
                    block.titleScore += 1
    
    """
    width of title block should not be too small
    """
    @staticmethod
    def titleRule3(block, page_width):
        if  block.width < (page_width/8):
             block.titleScore = 0
        else:
            block.titleScore += 1
    ...
    """
    position of title block should be relatively high
    """
    @staticmethod
    def titleRule4(block, page_height):
        if block.y < (page_height/8):
            block.titleScore += 2

    """
    position of title block should be include the center point of the page
    """
    @staticmethod
    def titleRule5(block,page_width):
        centerPoint = page_width/2
        if block.x < centerPoint < (block.x + block.width):
            block.titleScore += 2

    
    """
    content block should be located below title block
    """
    @staticmethod
    def contentRule1(block,title_block, page_height):
        if block.y < title_block.y:
            block.contentScore = 0
        elif block.y > title_block.y and block.height > (page_height*(1/5)):
            block.contentScore += 1
        
        """
        elif (title_block.y - block.y) < (page_height*0.08):
            block.contentScore += 1
        """
    """
    contains same child blocks with same font size
    """
    @staticmethod
    def contentRule2(block):
        sameFontSizeBlockCount = 0
        existingFontSizeList = []
        for childblock in block.children:
            for box in childblock.boxs:
                if box.nodeType == 3:
                    existingFontSizeList.append(float(box.visual_cues['font-size'].replace('px','')))
        if len(existingFontSizeList) != 0:
            mostCommonFontSize = Counter(existingFontSizeList).most_common(1)[0][0]
        for childblock in block.children:
            for box in childblock.boxs:
                if box.nodeType == 3:
                    if(mostCommonFontSize == float(box.visual_cues['font-size'].replace('px',''))):
                        sameFontSizeBlockCount += 1
        if (sameFontSizeBlockCount > 5):
            block.contentScore += 3
        elif (sameFontSizeBlockCount > 3):
            block.contentScore += 2
        elif (sameFontSizeBlockCount > 1):
            block.contentScore += 1

    """
    content blocks should contains the center point and should be relatively high
    """
    @staticmethod
    def contentRule3(block, page_width, page_height):
        centerPoint = page_width/2
        if not (block.x < centerPoint < (block.x + block.width)):
            block.contentScore = 0
        
        if block.y + block.height < (page_height*(2/3)):
            block.contentScore += 1
        
        if block.height > (page_height*(1/4)):
                block.contentScore += 2
