...
class Vips:
    ...
    def service(self):
        ...
        be = BlockExtraction()
        block = be.service(self.url, self.nodeList, self.browser.get_window_size())
        blockList = be.blockList
        i = 0
        while self.checkDoc(blockList) and i<self.Round:
            print ("blockList.size::", len(blockList))
            
            if BlockExtraction.title_block[0].id == '1':
                print(str(i)+"th website")
                sys.exit(0)

            self.imgOut.outBlock(blockList, self.fileName,i)
            self.imgOut.outBlock(BlockVo.blocksChosen, self.fileName, 11)
            self.imgOut.outBlock(BlockExtraction.important_blocks, self.fileName,12)
    ...
    def setDriver(self):
        ...
        # CHROMEDRIVER_PATH = os.path.realpath('..') + r"\\chromedriver_win32\\chromedriver.exe" # driver path
