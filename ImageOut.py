...
class ImageOut:
    ...
    def outBlock(self, block, fileName, i=0):
        ...
        for blockVo in block:
            if blockVo.isVisualBlock or i == 11 or i == 12:               
                ################ Rectangle ###################
        ...
        if i == 11:    
            saved_path = fileName + '_Block_SELECTED.png'
        elif i == 12:
            saved_path = fileName + '_Block_TITLE_CONTENT.png'
            ImageOut.outCSV(fileName, block, 12)
        else:
    ...
    @staticmethod
    def outCSV(fileName, blockList, i=0):
        ImageOut.content = ""
        ImageOut.getTextBlock(blockList[1])
        with open(fileName+'_csv_output_.csv', mode='w', encoding= 'utf-8') as csv_file:
            fieldNames = ['Title','Content']
            writer = csv.DictWriter(csv_file, fieldnames=fieldNames)

            writer.writeheader()
            writer.writerow({'Title':blockList[0].boxs[0].nodeValue, 'Content':ImageOut.content})

    @staticmethod
    def getTextBlock(block):
        if not block.isVisualBlock:
            for b in block.children:
                ImageOut.getTextBlock(b)
        else:
             for box in block.boxs:
                if box.nodeType == 3:
                    if box.parentNode.nodeName != "script" and box.parentNode.nodeName != "noscript" and box.parentNode.nodeName != "style":
                        if not box.nodeValue.isspace() or box.nodeValue == None:
                            ImageOut.content += str(box.nodeValue)
